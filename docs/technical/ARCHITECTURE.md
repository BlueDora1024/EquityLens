...
6. Longbridge Quant 未命中项最多 4 路并发，并以 125ms 间隔平滑启动直连请求；富途通过 OpenD 限频分页读取原始行情；
...
仍按“证券 × 周期”请求，但以 4 路有限并发执行。RS 成员只返回 close；拐点的
...
连续 8 个同类基础设施错误、最近 20 个完成任务中同类错误至少 16 个、同一运行
第二次 429，或认证、权限、配额、存储、数据库损坏、内存不足等致命错误都会停止
